...
export function CreatorAccountSettings({workspace,navigate}:{workspace:WorkspaceData|null;navigate:(section:"profile"|"contracts")=>void}) {
  const [avatarFile,setAvatarFile]=useState<File|null>(null),[avatarPreview,setAvatarPreview]=useState(""),[avatarBusy,setAvatarBusy]=useState(false),[avatarMessage,setAvatarMessage]=useState(""),[avatarFailed,setAvatarFailed]=useState(false);
  ...
  async function saveAvatar(){if(!avatarFile||avatarBusy)return;setAvatarBusy(true);setAvatarMessage("");setAvatarFailed(false);try{const r=await fetch("/api/creator-auth/avatar",{method:"POST",headers:{"Content-Type":avatarFile.type},body:avatarFile});const d=await r.json();if(!r.ok)throw Error(d.message||"頭像保存失敗");setAvatarFile(null);setAvatarMessage("頭像已更新");window.dispatchEvent(new Event("creator-avatar-updated"));}catch(e){setAvatarFailed(true);setAvatarMessage(e instanceof Error?e.message:"頭像保存失敗，請重試");}finally{setAvatarBusy(false);}}
  ...
  return <section className="cw-card rp-panel ca-account-page">
    ...
    <section className="ca-account-section"><h3>基本資料</h3><dl className="ca-account-rows">
      <div><dt>賬號頭像</dt><dd><span className="ca-account-avatar"><CreatorAvatar/></span><div className="ca-avatar-editor"><label className="ca-account-link">{avatarBusy?"正在保存…":"修改頭像"}<input aria-label="選擇頭像" type="file" accept="image/jpeg,image/png,image/webp" disabled={avatarBusy||!account} onChange={e=>{const file=e.currentTarget.files?.[0];e.currentTarget.value="";if(!file)return;setAvatarMessage("");setAvatarFailed(false);if(!["image/jpeg","image/png","image/webp"].includes(file.type)||file.size>5*1024*1024||file.size===0){setAvatarFailed(true);setAvatarMessage("請選擇 5 MB 以內的 JPG、PNG 或 WebP 圖片");return;}setAvatarFile(file);}}/></label><small>JPG、PNG、WebP，不超過 5 MB</small>{avatarPreview&&<div className="ca-avatar-preview"><img src={avatarPreview} alt="新頭像預覽"/><span>將居中裁切為正方形</span><button className="cw-primary" disabled={avatarBusy} onClick={()=>void saveAvatar()}>{avatarBusy?"保存中…":"保存頭像"}</button><button className="ca-account-cancel" disabled={avatarBusy} onClick={()=>{setAvatarFile(null);setAvatarMessage("");}}>取消</button></div>}{avatarMessage&&<p role={avatarFailed?"alert":"status"} className={avatarFailed?"ca-account-error":"ca-account-success"}>{avatarMessage}</p>}</div></dd></div>
    ...
    <section className="ca-account-section rp-linked-accounts"><header className="rp-heading"><h3>我的 App 賬號</h3>{!account?.appUserId&&<button className="cw-primary" disabled={busy||!account} onClick={()=>{setMode("binding");setMessage("");}}>添加賬號</button>}</header><div className="rp-table-wrap"><table className="rp-table"><thead><tr><th>賬號信息</th><th>App UID</th><th>綁定狀態</th><th>操作</th></tr></thead><tbody>{account?.appUserId?<tr><td><div className="rp-work"><span className="ca-account-avatar"><UserRound size={24}/></span><strong>{account.appUsername||"MY DREAM App 賬號"}</strong></div></td><td>{account.appUserId}</td><td><span className="rp-status is-success">已綁定</span></td><td><div className="rp-row-actions"><button className="rp-link" disabled={busy} onClick={()=>{setMode("binding");setMessage("");}}>重新驗證</button><button className="rp-link" disabled={busy} onClick={()=>{setMode("unbind");setMessage("");}}>解除綁定</button></div></td></tr>:<tr><td colSpan={4} className="rp-account-empty">{account?"尚未綁定 App 賬號":"正在讀取賬號…"}</td></tr>}</tbody></table></div>
