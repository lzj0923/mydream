...
import {Building2,Users,UserPlus,CheckCircle2,ShieldCheck,LoaderCircle} from "lucide-react";
...
export function CompanyTeam(){
 ...
 return <section className="cw-card ct-team"><header className="ct-heading"><span className="ct-brand-icon"><Building2 size={23}/></span><div><h1 className="cw-page-title">公司與團隊</h1><p>{data?.member?`當前公司：${data.name}`:data?.company?`${data.name} · 總管理員`:"使用個人賬號接受公司邀請，加入後共同管理公司項目。"}</p></div>{data?.company&&!data.member&&<span className="ct-role"><ShieldCheck size={13}/>總管理員</span>}</header>
 ...
 {data?.company&&!data.member&&!data.unavailable&&<><form className="ct-invite" aria-busy={pending==="invite"} onSubmit={async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form);if(await act("invite",{account:String(f.get("account")||"").trim(),role:inviteRole,permissions:invitePermissions}))form.reset();}}>
 ...
 <footer className="ct-invite-footer"><span><ShieldCheck size={16}/>收益、銀行卡、提現、簽約及企業管理僅總管理員可用。</span><button className="cw-primary" disabled={busy}>{pending==="invite"?<LoaderCircle className="ct-spinner" size={16}/>:<UserPlus size={16}/>} {pending==="invite"?"正在發送…":"發送邀請"}</button></footer></fieldset></form>
 <section className="ct-roster"><div className="ct-section-title"><Users size={19}/><h3>團隊成員</h3><span className="ct-count">已加入 {data.members?.filter(m=>m.state==="ACTIVE").length||0}</span><span className="ct-count">待接受 {data.members?.filter(m=>m.state==="INVITED").length||0}</span></div>{data.members?.map(m=><MemberRow key={`${m.accountId}:${m.state}:${m.canEdit}:${m.canUpload}:${m.canSubmit}:${m.role}:${m.extraPermissions}`} member={m} busy={busy} change={body=>act("member",body)}/>)}{!data.members?.length&&<div className="ct-empty"><Users size={26}/><strong>還沒有團隊成員</strong><p>發出第一份邀請，在這裡查看接受狀態並管理權限。</p></div>}</section>
